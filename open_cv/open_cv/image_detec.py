...
class ImageSubscriber(Node):
    ...
    # rint(gray)
        ...
        gray2 = cv2.GaussianBlur(gray, (19, 19), 1)
        ...
        canny = cv2.Canny(gray, 10, 1200)
        p2 = cv2.dilate(canny,kernel)

        ret, thresh = cv2.threshold(gray, 140, 220, cv2.THRESH_BINARY)
    ...
    # try: ? if there is no circle, output typeerror
        for circle in circles[0]:
            ...
            cv2.circle(gray, (x, y), r, (0, 0, 255), 3)
    ...
    # # print(hierarchy[0])
        ...
        #         print(cX, cY)
        #         col = cv2.cvtColor(p2, cv2.COLOR_GRAY2BGR)
