...
class AutoCalibration(Node):
    ...
    def __init__(self,):
        ...
        # global self.Parameter, self.mode 
        # self.mode = '2'
        # self.Parameter = ('/joint_trajectory_controller/state',
        #         '/joint_trajectory_controller/follow_joint_trajectory',
        #         '/Cam2/image_raw',
        #         [-0.359, -0.0458, -0.051544, 0.0],
        #         [-0.359, -0.0458, -0.051544, 6.2],
    ...
    def state_callback(self, msg):
        ...
        if msg.desired.positions.tolist()[3] == self.Parameter[5][0] and msg.actual.positions[3] > self.Parameter[5][0]-0.000001 and msg.actual.positions[3] < self.Parameter[5][0]+0.000001:
              # msg.desired.positions.tolist()[3] == self.Parameter[5][0] and 
...
# UnboundLocalError: local variable 'col' referenced before assignment
        for i in range(len(contours)):  #sobel? kaolv geng fuza yidian
            ...
            if len(contours[i]) >= 100 and len(contours[i]) <= 500:
                # sd = np.array(contours[i],dtype=float)
                # for m in sd:
                #     x.append(m)
                # #sd = np.array(contours[i]).tolist()
                # sd2 = taubinSVD(x)
                step = 2
                ...
                for n in contours[i]:
                    for l in n:
                        for k in l:
                            x.append(k)
                list__ = [x[q:q+step] for q in range(0, len(x), step)]
 
                for iii in list__:
                        y.append(iii)

                        #df=np.array(l,dtype=np.float32).tolist()
                f,g,h,j = taubinSVD(y)
                # print(contours[i])
                print('------------')
                
                if h > 105.0 and h < 108.0:
                ...
                # cv2.circle(im, (int(retval[0][0]),int(retval[0][1])),1, (0, 0, 255), -2)
                    ...
                    #cv2.ellipse(self.col, retval, (0, 0, 255), thickness=1) 
                    #cv2.circle(self.col, (int(retval[0][0]),int(retval[0][1])),1, (0, 0, 255), -2)

                    cv2.circle(self.col, (int(f),int(g)),1, (0, 0, 255), -2)
                    cv2.circle(self.col, (int(f),int(g)),int(h), (0, 0, 255), 2)
                    a.append((f,g,h,j))
                    b += f
                    c += g
        ...
        self.list.append([f,g])
        self.RR = self.r_r/len(self.list)
        self.mm = self.m/len(self.list) 
              
        for point in self.list:
    
            cv2.circle(self.col, (int(point[0]),int(point[1])),1, (0, 0, 255), -2)
        print('-------------------------------------')
    ...
    def fit_ellipse(self): 
        ...
        #print('%s,%s,%s,%s'%(self.x,self.y,e,r))
        #img =np.zeros((2000, 2000, 3), np.uint8)
        # cv2.ellipse(self.col, data, (0, 0, 255), thickness=1) 
        cv2.circle(self.col, (int(data[0][0]),int(data[0][1])),1, (0, 0, 255), -2)
        cv2.circle(self.col, (int(data[0][0]),int(data[0][1])),int((data[1][0]/2+data[1][1]/2)/2), (0, 0, 255), -2)
    ...
    # AttributeError: 'ImageSubscriber' object has no attribute 'x'
        ...
        with open ("/home/pmlab/ros2_ws/src/match_pm_robot/pm_robot_description/calibration_config/pm_robot_joint_calibration.yaml"
            ...
            joint_calibration['PM_Robot_Tool_TCP_Joint']['x_offset'] = float(self.error[0]*self.pixel_size)  # error(0)-self.x
            joint_calibration['PM_Robot_Tool_TCP_Joint']['y_offset'] = float(-self.error[1]*self.pixel_size)
